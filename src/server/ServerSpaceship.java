...
public class ServerSpaceship {
    ...
    public void update(float dt) {
        ...
        if (shoot && !wasShooting && shootCooldown <= 0) {
            wantsToShoot = true;
        ...
        if (position.x < -size)
            position.x = Constants.WORLD_WIDTH + size;
        if (position.x > Constants.WORLD_WIDTH + size)
            position.x = -size;
        if (position.y < -size)
            position.y = Constants.WORLD_HEIGHT + size;
        if (position.y > Constants.WORLD_HEIGHT + size)
            position.y = -size;
