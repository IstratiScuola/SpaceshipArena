...
import common.network.GameOverPacket;
import common.network.LobbyStatePacket;

import static org.lwjgl.opengl.GL11.*;

//sezione fatta in parte da IA, come scritto in relazione
...
public class Renderer {
    ...
    public void renderSpaceship(ClientSpaceship ship, boolean isLocalPlayer) {
        ...
        float alpha = invulnerable ? (float) (0.5f + 0.5f * Math.sin(System.currentTimeMillis() * 0.02)) : 1.0f;
    ...
    public void renderLobby(LobbyStatePacket lobbyState, int localPlayerId, GameOverPacket gameOver) {
        float centerX = Constants.WORLD_WIDTH / 2f;
        float centerY = Constants.WORLD_HEIGHT / 2f;

        // Background with stars effect
        for (int i = 0; i < 50; i++) {
            float starX = (i * 37 + System.currentTimeMillis() / 100) % Constants.WORLD_WIDTH;
            float starY = (i * 73) % Constants.WORLD_HEIGHT;
            float starSize = 1 + (i % 3);

            glBegin(GL_QUADS);
            glColor4f(1.0f, 1.0f, 1.0f, 0.3f + (i % 5) * 0.1f);
            glVertex2f(starX, starY);
            glVertex2f(starX + starSize, starY);
            glVertex2f(starX + starSize, starY + starSize);
            glVertex2f(starX, starY + starSize);
            glEnd();
        }
        ...
        float panelY = centerY - panelHeight / 2 - 30;
        ...
        glVertex2f(panelX, panelY + panelHeight - 50);
        glVertex2f(panelX + panelWidth, panelY + panelHeight - 50);
        glVertex2f(panelX + panelWidth, panelY + panelHeight);
        glVertex2f(panelX, panelY + panelHeight);
        glEnd();

        // Title text
        BitmapFont.drawTextCentered("SPACE ARENA", centerX, panelY + panelHeight - 38, 2.0f, 1.0f, 1.0f, 1.0f, 1.0f);

        // Show game over message if applicable
        if (gameOver != null) {
            float gameOverY = panelY + panelHeight - 100;

            // Winner announcement box
            glBegin(GL_QUADS);
            glColor4f(0.8f, 0.6f, 0.1f, 0.9f);
            glVertex2f(centerX - 200, gameOverY);
            glVertex2f(centerX + 200, gameOverY);
            glVertex2f(centerX + 200, gameOverY + 45);
            glVertex2f(centerX - 200, gameOverY + 45);
            glEnd();

            // Winner text
            BitmapFont.drawTextCentered("VINCITORE: " + gameOver.winnerName.toUpperCase(), centerX, gameOverY + 15, 1.5f,
                    1.0f, 1.0f, 0.3f, 1.0f);
        }

// Player list
if (lobbyState != null && lobbyState.players != null) {
    float listY = panelY + panelHeight - (gameOver != null ? 160 : 110);
    float lineHeight = 40;

    for (int i = 0; i < lobbyState.players.size(); i++) {
        LobbyStatePacket.LobbyPlayer player = lobbyState.players.get(i);
        float rowY = listY - (i + 1) * lineHeight;
        ...
        glVertex2f(panelX + 20, rowY - 5);
        glVertex2f(panelX + panelWidth - 20, rowY - 5);
        glVertex2f(panelX + panelWidth - 20, rowY + 30);
        glVertex2f(panelX + 20, rowY + 30);
        glEnd();

        // Ready indicator box
        float indicatorX = panelX + 40;
        ...
        glVertex2f(indicatorX, rowY + 5);
        glVertex2f(indicatorX + indicatorSize, rowY + 5);
        glVertex2f(indicatorX + indicatorSize, rowY + 25);
        glVertex2f(indicatorX, rowY + 25);
        glEnd();
        ...
        if (player.ready) {
            ...
            glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
            // Classic checkmark: left-bottom → center → right-top
            glVertex2f(indicatorX + 5, rowY + 12);
            glVertex2f(indicatorX + 10, rowY + 17);
            glVertex2f(indicatorX + 15, rowY + 7);
            glEnd();
        }

        // Ready/Waiting text
        String status = player.ready ? "PRONTO" : "NON PRONTO";
        float statusR = player.ready ? 0.3f : 0.8f;
        float statusG = player.ready ? 1.0f : 0.8f;
        float statusB = player.ready ? 0.3f : 0.3f;
        BitmapFont.drawText(status, indicatorX + 30, rowY + 8, 1.0f, statusR, statusG, statusB, 1.0f);

        // Player name - right aligned
        float nameR = isLocal ? 0.5f : 0.9f;
        float nameG = isLocal ? 0.9f : 0.9f;
        float nameB = isLocal ? 1.0f : 0.9f;

        // Calculate position for right alignment
        float rightMargin = 20; // Padding from right edge
        float nameX = panelX + panelWidth - rightMargin - BitmapFont.getTextWidth(player.playerName.toUpperCase(), 1.2f);
        BitmapFont.drawText(player.playerName.toUpperCase(), nameX, rowY + 8, 1.2f, nameR, nameG, nameB, 1.0f);

        // (YOU) indicator for local player - position it to the left of the name
        if (isLocal) {
            float teX = nameX - BitmapFont.getTextWidth("(TE)", 1.0f) - 5; // 5 pixels spacing
            BitmapFont.drawText("(TE)", teX, rowY + 8, 1.0f, 0.3f, 0.8f, 0.3f, 1.0f);
        }
    }

            // Instructions panel at bottom
            float instructY = panelY + 20;

            glBegin(GL_QUADS);
            glColor4f(0.15f, 0.2f, 0.3f, 0.9f);
            glVertex2f(panelX + 20, instructY);
            glVertex2f(panelX + panelWidth - 20, instructY);
            glVertex2f(panelX + panelWidth - 20, instructY + 70);
            glVertex2f(panelX + 20, instructY + 70);
            glEnd();

            // Instructions text
            BitmapFont.drawTextCentered("PREMI SPAZIO QUANDO PRONTO", centerX, instructY + 40, 1.2f, 0.8f, 0.8f, 1.0f,
                    1.0f);

            // Minimum players warning
            if (lobbyState.players.size() < 2) {
                BitmapFont.drawTextCentered("IN ATTESA DI ALTRI GIOCATORI", centerX, instructY + 15, 1.0f, 1.0f, 0.6f,
                        0.3f, 1.0f);
            } else {
                // Check if all ready
                boolean allReady = true;
                for (LobbyStatePacket.LobbyPlayer p : lobbyState.players) {
                    if (!p.ready) {
                        allReady = false;
                        break;
                    }
                ...
                if (allReady) {
                    BitmapFont.drawTextCentered("INIZIA", centerX, instructY + 15, 1.0f, 0.3f, 1.0f,
                            0.3f, 1.0f);
                } else {
                    BitmapFont.drawTextCentered("IN ATTESA DI TUTTI I GIOCATORI", centerX, instructY + 15, 1.0f, 0.8f, 0.8f,
                            0.5f, 1.0f);
                }
            }

            /*// Player count in title bar
            String countText = lobbyState.players.size() + " PLAYER" + (lobbyState.players.size() != 1 ? "S" : "");
            BitmapFont.drawText(countText, panelX + panelWidth - 120, panelY + panelHeight - 38, 1.0f, 0.7f, 0.7f, 0.8f,
                    1.0f);*/
        }
    }
}